...
Spec only in this repo - wiring it to a real headless-browser tool (e.g., Playwright) is left to the implementer's environment. See [`usability-heuristics-reviewer`](../../skills/usability-heuristics-reviewer) for the underlying heuristics library, and [`release-notes-agent`](../../../launch-and-growth/agents/release-notes-agent) for this toolkit's one fully runnable, credential-free agent.
