...
Spec only in this repo - wiring it to a specific analytics backend (Amplitude, Mixpanel, GA, a warehouse) requires that org's credentials. See [`release-notes-agent`](../../08-product-launch-growth-and-monetization/release-notes-agent) for a fully runnable example of the same agent pattern using only local, credential-free data.
